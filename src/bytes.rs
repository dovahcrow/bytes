...
pub(crate) struct Vtable {
    ...
    pub to_mut: unsafe extern "C" fn(&AtomicPtr<()>, *const u8, usize) -> BytesMut,
    /// fn(data)
...
impl From<Bytes> for Vec<u8> {
    fn from(bytes: Bytes) -> Vec<u8> {
        let bytes = ManuallyDrop::new(bytes);
        unsafe { (bytes.vtable.to_vec)(&bytes.data, bytes.ptr, bytes.len) }.into_vec()
    }
}
// ===== impl Vtable =====
...
unsafe extern "C" fn static_to_mut(_: &AtomicPtr<()>, ptr: *const u8, len: usize) -> BytesMut {
    let slice = slice::from_raw_parts(ptr, len);
    BytesMut::from(slice)
}
...
unsafe extern "C" fn owned_clone(data: &AtomicPtr<()>, ptr: *const u8, len: usize) -> Bytes {
...
unsafe extern "C" fn owned_to_vec(data: &AtomicPtr<()>, ptr: *const u8, len: usize) -> RVec<u8> {
    let slice = RSlice::from_raw_parts(ptr, len);
    let vec = slice.to_rvec();
    owned_drop_impl(data.load(Ordering::Relaxed));
    vec
}

unsafe extern "C" fn owned_to_mut(data: &AtomicPtr<()>, ptr: *const u8, len: usize) -> BytesMut {
    BytesMut::from_rvec(owned_to_vec(data, ptr, len))
}

unsafe extern "C" fn owned_is_unique(_data: &AtomicPtr<()>) -> bool {
    false
}
...
unsafe extern "C" fn owned_drop(data: &mut AtomicPtr<()>, _ptr: *const u8, _len: usize) {
    let owned = data.load(Ordering::Relaxed);
...
unsafe extern "C" fn promotable_even_to_mut(
    data: &AtomicPtr<()>,
    ptr: *const u8,
    len: usize,
) -> BytesMut {
...
unsafe extern "C" fn promotable_odd_to_mut(
    data: &AtomicPtr<()>,
    ptr: *const u8,
    len: usize,
) -> BytesMut {
...
unsafe extern "C" fn shared_to_mut(data: &AtomicPtr<()>, ptr: *const u8, len: usize) -> BytesMut {
